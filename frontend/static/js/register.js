...
function startCapture() {
    ...
    navigator.mediaDevices.getUserMedia({ video: true })
        .then(stream => {
            ...
            video.play();
            document.getElementById("status").innerText = "📸 Capturing faces... Please stay steady.";
...
function captureFrame(id, name) {
    ...
    fetch("/capture_face", {
        ...
        body: JSON.stringify({ id: id, name: name, image: canvas.toDataURL("image/jpeg") })
    })
    .then(res => res.json())
    .then(data => {
        document.getElementById("status").innerText = `📊 Captured ${data.count} / 30 frames`;
        ...
        if (data.done) {
            ...
            isCapturing = false;
            document.getElementById("status").innerText = "✅ Capture complete. Ready to train.";
...
function trainModel() {
    ...
    fetch("/train_model", { method: "POST" })
        ...
        .then(data => {
            if (data.success) {
                document.getElementById("status").innerText = "🎉 Model trained successfully!";
            } else {
